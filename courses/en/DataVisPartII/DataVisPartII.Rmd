...
In this tutorial we will learn how to use `ggplot2` to make beautiful plots of your data. If you want to know how to make basic plots in R then have a look at the tutorial [Data Visualisation Part I](../DataVisPartI/DataVisPartI).
...
In the next 4 sections we will learn how to reproduce the same type of plots that we made in [Data Visualisation Part I](../DataVisPartI/DataVisPartI) using `ggplot2`.
...
Now we can create an object that holds the main elements of a plot, the `data` and the `aes`.
...
The function `geom_point` tells `ggplot2` to visualise the data as points. The `geom_` functions will inherits any aesthetics parameters from `aes`, unless overwritten. For example, we can specify that we want different colours for points belonging to different species.
...
We can also add a smoother to visualise the relationship more clearly. Here we use a linear model to calculate the smoother, but you can use other methods too (for exampl `gam`).
...
We can now see the distribution of the variable `Sepal.Length` for every species.
...
I hope that by now you see the advantages of using `ggplot2`. This tutorial is just a basic introduction to this package and there are many more types of visualisations that you can do with it, such as heatmaps, bubble plots, maps of spatial data etc.
